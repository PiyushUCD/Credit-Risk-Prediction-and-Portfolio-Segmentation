...
class Visualizer:
    ...
    def plot_calibration_curves(self, evaluation_results: dict[str, dict]) -> str:
        """Plot calibration curves (reliability diagrams) for all evaluated models."""
        fig, ax = plt.subplots(figsize=(8, 6))
        ...
        for name, res in evaluation_results.items():
            ...
            brier = res["metrics"].get("brier")
            label = f"{name}"
            if brier is not None:
                label += f" (Brier={float(brier):.3f})"
            ax.plot(mean_pred, frac_pos, marker="o", linewidth=1, label=label)
        ...
        path = os.path.join(self.output_dir, "07_calibration_curves.png")
        fig.tight_layout()
        fig.savefig(path, dpi=int(self.config.get("PLOTS_DPI", 300)), bbox_inches="tight")
        plt.close(fig)
        return path
