...
def main() -> None:
    ...
    viz.plot_calibration_curves(eval_results)
    viz.plot_portfolio(portfolio.portfolio_table)
    viz.plot_probability_distribution(y_all, best_probs_all)

    # Optional: SHAP global summary (if shap is installed)
    try:
        from credit_risk.explainability import try_shap_global_summary, unwrap_estimator

        base = unwrap_estimator(scoring_model)
        if hasattr(base, "named_steps"):
            # Use a small sample to keep runtime reasonable
            sample_n = int(cfg.get("SHAP_SAMPLE_SIZE", 1500))
            X_s = X_all.sample(
                min(sample_n, len(X_all)),
                random_state=int(cfg.get("RANDOM_STATE", 42)),
            )
            out_png = os.path.join(output_dir, "08_shap_summary.png")
            _, _msg = try_shap_global_summary(base, X_s, out_png=out_png)
            if _msg:
                print(f"ℹ️ SHAP summary skipped: {_msg}")
    except Exception:
        pass
